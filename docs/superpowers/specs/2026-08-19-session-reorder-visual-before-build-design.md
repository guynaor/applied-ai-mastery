# Session reorder — visual first, app last
...
Guy taught the course and found the order does not flow. Two pieces of
learner feedback, the second arriving after the first was specced:

1. The visual session — generating images, video, narration and a deck —
   should come **before** the build sessions.
2. The app-building session should come **last** of the taught sessions.

Together these are a single swap: the visual session and the app session
trade places. The object session does not move.
...
| 5 | Make a Space Work Better | 5 | design-physical-project |
| **6** | **Solve a Recurring Problem** | **4** | build-personal-tool |
| 7 | (capstone — separate spec) | 7 | workflow-portfolio-project |

Sessions 1, 2, 3, 5 and 7 do not move. Two sessions swap: **4 ↔ 6**.

This is a transposition, not a three-cycle, which makes it materially
cheaper than the first version of this spec: 20 files move rather than 31,
and every `session-05-*` file stays exactly where it is.

**A pedagogical consequence worth keeping:** the course now ends on the app
session, which is where the permission boundary and the stop rule live. The
last thing a learner builds is the thing that most needs bounding, and the
capstone's final deliverable is the same. That is a better closing note than
ending on a 3D model.
...
presentation, designed object, small app. The capstone spec at
...
- swaps `completed`, `journals` and the keys of `missions` by
  `{4:6, 6:4}`, leaving 1, 2, 3, 5 and 7 untouched;
...
lesson 9 (mini app) now maps to session 6 and lesson 11 (visual) to session
4; lesson 10 (room design) is unchanged at 5. It becomes
`…9:6, 10:5, 11:4, 12:7`.
...
A two-way swap, so renames must still go via a temporary name or git will
collide. Topic slugs inside filenames do not change. **Every
`session-05-*` file is untouched** — 11 files that do not move.
...
- `personal-course/sessions/session-04-build-personal-tool.md` → `session-06-…`
- `personal-course/he/sessions/session-04-build-personal-tool.md` → `session-06-…`
- `personal-course/instructor/sessions/session-04-guide.md` → `session-06-guide.md`
- `personal-course/he/instructor/sessions/session-04-guide.md` → `session-06-guide.md`
- `personal-course/instructor/samples/session-04-{slides,slides-he,test-cards,test-cards-he}.html` → `session-06-…`

**Object, 5 → 5** — no files move. `session-05-setup.md` keeps its name, and
the object brief and guide keep every internal link they already have.
...
- Briefs link their own setup guide and samples. `session-05-setup.md` is
  referenced from both the object brief and its guide — and because the
  object session does not move, those references are **correct as they
  stand**. Do not touch them.
