...
5. design alert state, history, recovery, duplicate suppression, and re-arming.
...
### 45–72 min — Build a clean working table
...
### 72–88 min — Validate the cleaned result
...
### 88–108 min — Design the low-stock workflow
...
Require parse-failure handling and human approval for purchasing.

### 108–120 min — Debrief

Ask the student to explain why “Alert Sent” alone is insufficient state and why recovery must be represented explicitly.
...
The student produces a documented clean table that preserves both disputed observations, a usable data contract, an issue log, and an alert design that handles duplicate suppression, recovery, re-arming, and failures.
...
If spreadsheet software is unavailable, conduct the cleanup as a paper or text-based transformation plan using ten representative rows. Preserve the same reasoning and state-machine requirements.
